    ...
    class AuthMiddleware(BaseHTTPMiddleware):
        async def dispatch(self, request: Request, call_next):
            ...
            if LOCALHOST_BYPASS and _is_trusted_loopback(request):
                # Grant admin-equivalent identity so admin-only routes
                # (shell/exec, model-endpoints, etc.) work in desktop mode.
...
from fastapi.staticfiles import StaticFiles
# Mount Vite-built assets if a compiled frontend is present
_frontend_assets = abs_join(BASE_DIR, "frontend/dist/assets")
...
# from routes.live_routes import setup_live_routes
# app.include_router(setup_live_routes(api_key_manager))

# Jarvis Event SSE Stream
...
async def serve_index(request: Request):
    frontend_path = abs_join(BASE_DIR, "frontend/dist/index.html")
    if os.path.exists(frontend_path):
        return _serve_html_with_nonce(request, frontend_path)
