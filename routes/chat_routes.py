...
def setup_chat_routes(
    ...
    async def chat_stream(request: Request) -> StreamingResponse:
        ...
        is_code_mode = str(form_data.get("code_mode", "")).lower() == "true"
        if is_code_mode:
            chat_mode = "agent"
            allow_bash = "true"
        ...
        if chat_mode == "chat" and _tool_intent and _tool_intent.needs_tools:
            chat_mode = "agent"
            if _tool_intent.category != "shell":
                auto_escalated = True
            logger.info(
        ...
        disabled_tools = set()
        is_shell_auto_escalation = (not user_requested_agent) and (chat_mode == "agent") and (not auto_escalated)
        if is_shell_auto_escalation:
            # Do not disable bash on shell auto-escalation from chat mode
            pass
        else:
        ...
        if is_code_mode:
            # Disable non-coding tools to focus the model purely on programming
            disabled_tools.update({
                "web_search", "web_fetch", "builtin_browser", "generate_image",
                "manage_calendar", "manage_notes", "email", "search_chats", "manage_memory"
            })
        else:
        ...
        async def stream_with_save() -> AsyncGenerator[str, None]:
            ...
            if is_code_mode:
                messages = [{"role": "system", "content": "You are currently running in Code Mode. You are a hands-on developer agent, similar to Claude Code. If the user mentions 'Claude', 'Claude Code', or 'Claude's capabilities', understand that they are referring to your current operating mode. You have full access to local tools (bash, python, write_file, edit_file, computer) on this system. You MUST use your OWN local tools directly to write, edit, run, and verify code on the user's disk. Never say 'I cannot run this application' or 'I don't have access to Claude's capabilities'. Instead, ask for permission, run the code using your tools, and show the results. Focus on writing clean, correct, efficient, and well-structured code. Avoid conversational fluff and non-programming tasks."}] + messages
            ...
            if _is_image_generation_session(sess, owner=_user):
                ...
                try:
                    ...
                    from routes.event_routes import global_event_broadcaster
                    global_event_broadcaster.broadcast("workflow.started", {"objective": messages[-1].get("content") or "Agent execution"})
                    ...
                    try:
                        async for chunk in stream_agent_loop(
                            ...
                            if chunk.startswith("data: ") and not chunk.startswith("data: [DONE]"):
                                try:
                                    ...
                                    if "delta" in data:
                                        ...
                                        if data.get("type") == "agent_step":
                                            ...
                                            _agent_tool_calls += 1
                                            global_event_broadcaster.broadcast("task.started", {"title": f"Running tool: {data.get('tool')}"})
                                        yield chunk
                                    elif data.get("type") == "fallback":
                                        # Selected model failed; a fallback answered.
                                ...
                                global_event_broadcaster.broadcast("workflow.completed", {"objective": "Objective accomplished"})
                                if full_response:
                                    _saved_id = save_assistant_response(
                                        sess, session_manager, session, full_response, last_metrics,
                                        character_name=ctx.preset.character_name,
                        ...
                        global_event_broadcaster.broadcast("workflow.failed", {"error": str(e)})
                        raise
                except (asyncio.CancelledError, GeneratorExit):
    ...
    # ------------------------------------------------------------------ #
    # POST /api/chat/approve_tool — approve or reject a tool execution in Code Mode
    # ------------------------------------------------------------------ #
    @router.post("/api/chat/approve_tool")
    async def chat_approve_tool(request: Request) -> Dict[str, Any]:
        data = await request.json()
        approval_id = data.get("approval_id")
        approved = bool(data.get("approved", False))
        if not approval_id:
            raise HTTPException(400, "Missing approval_id")
        
        from src.tool_approvals import resolve_approval
