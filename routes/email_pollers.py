                        ...
                        if jm:
                            ...
                            if urgency in ("critical", "high"):
                                try:
                                    ...
                                    # Deep-link to open the original email in Spark (if public URL is configured).
                                    ...
                                    # HTML alternative with a clickable "Open in Spark" button
                                    import html as _h
...
def _scheduled_poll_once() -> dict:
    ...
    try:
        ...
        kind_expr = "spark_kind" if "spark_kind" in cols else "'scheduled' AS spark_kind"
        ...
        for r in rows:
            ...
            try:
                ...
                spark_kind = r[10] if len(r) > 10 else "scheduled"
                ...
                outer["X-Spark-Origin"] = "spark-ui"
                outer["X-Spark-Kind"] = re.sub(r"[^A-Za-z0-9_.-]", "-", spark_kind or "scheduled")[:64]
                outer["X-Spark-Ref"] = sid
                if r[6]:
...
async def _scheduled_email_poller():
    ...
    also exposed via the `spark-mail poll-scheduled` CLI for
...
def _inprocess_pollers_enabled() -> bool:
    """Honour `SPARK_INPROCESS_POLLERS` — set to `0`/`false`/`no`/`off`
    to disable the asyncio tasks so a cron / systemd-timer setup driving
    `spark-mail poll-scheduled` is the sole external driver. The legacy
    ...
    import os
    raw = os.environ.get("SPARK_INPROCESS_POLLERS", "1").strip().lower()
...
def _start_poller():
    ...
    Skipped entirely when `SPARK_INPROCESS_POLLERS=0` — use that when
    ...
    if not _inprocess_pollers_enabled():
        logger.info(
            "In-process email pollers disabled (SPARK_INPROCESS_POLLERS=0); "
            "drive `spark-mail poll-scheduled` externally."
        )
        return
    import asyncio
