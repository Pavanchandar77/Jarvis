...
_EXPLANATORY_PREFIX = re.compile(
    r"^\s*(?:how\s+(?:do|can)\s+i|can\s+you\s+explain|what\s+about|what\s+is|what\s+does|tell\s+me\s+(?:how|about)|show\s+me\s+how|explain\b)",
    re.I,
)
...
_ROUTING_PATTERNS: tuple[tuple[str, str, Pattern[str]], ...] = tuple(
    ...
    for category, reason, pattern in (
        ...
        ("shell", "git action request", r"\bgit\s+(status|diff|log|show|branch|commit|add|push|pull|clone|checkout|init|remote)\b"),
        ("shell", "repository file list request", r"\b(?:list|show|find)\s+(?:all\s+)?files\b"),
        ("shell", "git status request", r"\b(?:show|check|get)\s+(?:the\s+)?git\s+status\b"),
        # File operations (read/view)
        ("shell", "file read request", r"\b(read|view|cat|inspect|open|show)\s+(?:the\s+)?(?:code\s+in\s+)?[\w./\\-]+\.(?:py|js|ts|json|html|css|md|txt|sh|bat|yml|yaml|toml|ini)\b"),
        # File operations (write/edit)
        ("shell", "file write/edit request", r"\b(edit|write|change|modify|update|patch|fix)\s+(?:the\s+)?(?:code\s+in\s+)?[\w./\\-]+\.(?:py|js|ts|json|html|css|md|txt|sh|bat|yml|yaml|toml|ini)\b"),
        # Workspace/terminal commands
        ("shell", "workspace command request", r"\b(?:run|execute)\s+(?:a\s+)?(?:terminal|shell|bash|powershell|cmd)\s+command\b"),
    )
)
