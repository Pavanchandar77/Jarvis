...
Streaming agent loop for spark-ui.
...
_AGENT_PREAMBLE = """\
You are an AI assistant running locally on the user's machine via the Spark desktop application. You have direct access to their local filesystem, Git repository, GitHub via git CLI, the internet (via web_search/web_fetch), and local execution runtimes (like Python, virtual environments, or Docker) via the tools listed below.
Do NOT refuse by saying:
- "I cannot interact with your local file system or Git repository"
- "I do not have access to your computer" or "I am a local AI assistant and do not have access to your files"
- "I do not have the ability to browse the internet" or "I cannot access external websites"
- "I cannot connect to GitHub" (you can run `git` commands, use credentials, etc.)
- "I cannot run a sandbox or execute code" (you can run python or run docker/venv inside bash to sandbox/test code)
You DO have all these capabilities and tools.
To use a tool, write a fenced code block with the tool name as the language tag (e.g. ```bash, ```python, or ```web_search). The block executes automatically on the user's system and you see the output in the next turn."""
...
- Tool toggles like "turn off shell/search/research" use `ui_control toggle <name> <on|off>`, not memory.""",
    "sessions": """\
## Chat/session rules
- Spark chats are sessions. Use `list_sessions`/`manage_session`; do not shell out looking for chat files.
...
_DOMAIN_TOOL_MAP = {
    ...
    "files": {"bash", "python", "read_file", "write_file", "edit_file", "grep", "glob", "ls", "semantic_twin"},
...
`calendar` accepts a name ("Main") or short-id prefix.""",
    ...
    "semantic_twin": "- ```semantic_twin``` — Query the Semantic Twin knowledge graph for generated apps. PREFER this over re-reading the whole repository when you need architecture, dependency traces, component explanations, impact simulation, or concepts. Args (JSON): {\"action\": \"search|explain|trace_execution|trace_dependency|find_concept|simulate|architecture|status|list_projects|quiz|tutorial|timeline\", \"q\"?: \"...\", \"node_id\"?: \"...\", \"twin_id\"?: \"...\", \"project_id\"?: \"...\", \"mode\"?: \"senior\", \"proposal\"?: \"...\"}. After you generate an app with write_file, a twin is created automatically — use list_projects/status to find it.",
...
```
GENERIC LOOPBACK to allowed Spark internal endpoints. Use this whenever the user wants something the UI can do but there's NO named tool for it. Many UI buttons hit /api/* endpoints — you can hit allowed ones. Auth is handled automatically.
...
def _classify_agent_request(messages: List[Dict], last_user: str) -> Dict[str, object]:
    ...
    if has(r"\b(cookbook|serve|serving|served|launch|start|preset|vllm|sglang|llama\.?cpp|ollama|download|downloading|pull|cached models?|running models?|model servers?|models? (?:are )?running|what models?|model picker|gpu box|kierkegaard|spark|ajax|qwen|gemma|llama|mistral|minimax)\b"):
...
async def stream_agent_loop(
    ...
    if _endpoint_supports is None and endpoint_url and "nvidia.com" in endpoint_url.lower():
        _endpoint_supports = False
    ...
    # Semantic Twin Phase-1: track this agent turn so disk writes auto-build a twin.
    if not guide_only:
        try:
            from services.semantic_twin.integration.hooks import on_agent_turn_start
            _user_prompt_st = ""
            for _m in reversed(messages or []):
                if _m.get("role") == "user":
                    _c = _m.get("content")
                    _user_prompt_st = _c if isinstance(_c, str) else str(_c or "")
                    break
            on_agent_turn_start(
                ...
                workspace=workspace,
                model=model,
                backend=endpoint_url or "",
                user_prompt=_user_prompt_st,
                planning_prompt="",
                approved_plan=approved_plan or "",
            )
        except Exception:
            logger.debug("semantic twin turn start failed", exc_info=True)
    ...
    if messages and messages[-1].get("role") == "user" and not _is_api_model:
        _reminder = (
            "\n\n[SYSTEM REMINDER: You are running locally inside the Spark desktop application. "
            "You have full access to run bash commands, read/write local files, check git, and browse the web "
            "via the web_search/web_fetch tools. If you need to search the web, check git, or list files, "
            "you MUST output a tool code block (e.g. ```web_search or ```bash) and nothing else. "
            "Do NOT refuse or say you cannot access the internet or files.]"
        )
        messages_copy_for_api = []
        for i, msg in enumerate(messages):
            if i == len(messages) - 1:
                messages_copy_for_api.append({
                    "role": msg["role"],
                    "content": (msg.get("content") or "") + _reminder,
                    **{k: v for k, v in msg.items() if k not in ["role", "content"]}
                })
            else:
                messages_copy_for_api.append(msg)
        messages = messages_copy_for_api
    ...
    for round_num in range(1, max_rounds + 1):
        ...
        for i, block in enumerate(tool_blocks):
            ...
            needs_approval = (is_code_mode and block.tool_type in (
                "bash", "python", "write_file", "replace_file_content", "multi_replace_file_content"
            )) or (block.tool_type == "computer")
            approved = True
            if needs_approval:
                import uuid
                from src.tool_approvals import register_approval, get_approval_status
                approval_id = f"approval_{uuid.uuid4().hex}"
                evt = register_approval(approval_id)
                yield (
                    f'data: {json.dumps({"type": "tool_approval_request", "tool": block.tool_type, "command": cmd_display, "approval_id": approval_id, "round": round_num})}\n\n'
                )
                logger.info(f"Code Mode: tool approval requested for tool={block.tool_type}, approval_id={approval_id}")
                await evt.wait()
                approved = get_approval_status(approval_id)
                logger.info(f"Code Mode: tool approval response for approval_id={approval_id} is approved={approved}")
            ...
            if not approved:
                ...
                logger.info("Tool execution rejected by user in Code Mode: %s", block.tool_type)
    ...
    # Semantic Twin Phase-1: auto-generate/update twin when the agent wrote files.
    if not guide_only:
        try:
            from services.semantic_twin.integration.hooks import on_agent_turn_end
            _twin_result = on_agent_turn_end(
                session_id=session_id,
                owner=owner,
                workspace=workspace,
            ...
            if _twin_result and _twin_result.get("ok"):
                yield (
                    f'data: {json.dumps({"type": "semantic_twin", "data": _twin_result})}\n\n'
                )
                # Visible one-liner so users/agents know the twin exists
                _tid = _twin_result.get("twin_id")
                _pid = _twin_result.get("project_id")
                _note = (
                    f"\n\n_Semantic Twin ready — project `{_pid}` "
                    f"(twin `{_tid}`, r{_twin_result.get('content_revision')}). "
                    f"[Open explorer](/semantic-twin?twin={_tid})_\n"
                )
                yield f'data: {json.dumps({"delta": _note})}\n\n'
                full_response += _note
        except Exception:
            logger.debug("semantic twin turn end failed", exc_info=True)
