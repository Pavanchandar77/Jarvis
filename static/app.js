...
// Desktop window tiling â€” drag a modal near an edge/corner to snap.
import './js/tileManager.js';
import themeModule from './js/theme.js';
// IMPORTANT: import cookbook.js with NO ?v= query â€” the same plain specifier
...
// Default chat config â€” refreshed on every new-chat action so settings
...
//  document.getElementById('chat-form').addEventListener('submit', chatModule.handleChatSubmit);
  ...
  // Message count in the header â€” recount on any DOM change in
  // #chat-history and write "Â· N msgs" next to the title. Counts top-
  ...
  if (_metaCountEl && _chatHistEl) {
    ...
    const _updateMsgCount = () => {
      ...
      _metaCountEl.textContent = n ? `Â· ${n} msg${n === 1 ? '' : 's'}` : '';
    };
  ...
  // â”€â”€ Unified popup dismissal â”€â”€
  // Lightweight popups (header dropdowns, kebab menus, pickers) should vanish
  // on any "other action" â€” opening the sidebar, opening a tool window, etc.
  ...
  // windows are deliberately NOT touched here â€” those close via their own
  // controls.
  ...
  // rows, section headers) count as "other actions" â€” dismiss popups when one
  ...
  if (exportDlBtn && exportMenu) {
    ...
    // for a hiddenâ†’visible transition rather than hooking each one.
    ...
    for (const child of box.children) {
      if (child.classList?.contains('msg')) {
        ...
        for (const n of child.querySelectorAll('.agent-thread-node')) {
          ...
          if (cmd) line += `\n  cmd: ${cmd}`;
          if (output) {
            const truncated = output.length > 2000 ? output.slice(0, 2000) + 'â€¦' : output;
            line += `\n  out: ${truncated}`;
          }
          lines.push(line);
        }
  ...
  if (exportCopyBtn) {
    exportCopyBtn.addEventListener('click', async (e) => {
      ...
      // A new/empty chat has nothing to copy â€” don't write an empty string and
  ...
  if (exportPdfBtn) {
    exportPdfBtn.addEventListener('click', (e) => {
      ...
      const sessionName = meta ? meta.name : 'Spark';
  ...
  if (exportRenameBtn) {
    exportRenameBtn.addEventListener('click', (e) => {
      ...
      // A brand-new chat has no session id yet â€” still allow renaming if there's
  ...
  if (saveCustomPreset) {
    saveCustomPreset.addEventListener('click', async () => {
      // Skip character save when Group tab is active â€” group.js handles it
  ...
  // Settings dropdown removed â€” items are now inline in sidebar section
  ...
  document.addEventListener('keydown', (e) => {
    if (e.key === 'Escape') {
      ...
      // Scoreboard sits on top of the compare window â€” close it first.
      ...
      // Model picker popup â€” close before opening any modals
      ...
      // Map modal id â†’ sidebar list-item id to clear active state
      const modalItemMap = {
        'cookbook-modal': null,
      ...
      // No modals/popups open â€” minimize the document panel if open.
      // Esc should tab the doc down to a dock chip (same as the chevron),
      // NOT fully close it â€” closePanel('down') registers the chip +
  ...
  // â”€â”€ Shared modal dismiss helper â”€â”€
  ...
  // Click outside modal content â†’ close modal
  document.addEventListener('click', (e) => {
  ...
  // â”€â”€ Helper: start a fresh chat (deselect current, clear history, show welcome) â”€â”€
  function _startFreshChat() {
    ...
    // Reset research overflow dot (but don't touch research state â€” caller manages that)
  ...
  // â”€â”€ Close compare if active (used by all tool/sidebar activations) â”€â”€
  ...
  // â”€â”€ Tools section click handlers â”€â”€
  ...
  if (toolCookbookBtn) {
    toolCookbookBtn.addEventListener('click', async () => {
      ...
      // Try minimizedâ†’restore or openâ†’minimize via the manager first
      const Modals = await import('./js/modalManager.js');
      if (!Modals.toggle('cookbook-modal')) {
        // Not registered yet â†’ fresh open
        cookbookModule.open();
      }
    });
  }
  ...
  if (toolCalendarBtn) {
    toolCalendarBtn.addEventListener('click', async () => {
      ...
      // returns false when nothing is registered â†’ open fresh.
  ...
  // URL-based panel routing â€” bookmark /calendar, /notes, /cookbook etc
  ...
  // in its place. The two are mutually exclusive â€” sidebar-layout.js:57
  ...
  // hamburger click, or our own _restoreSidebarIfRouteCollapsed call â€”
  ...
  const _routeOpen = {
    ...
    '/email':    () => {
      ...
      // Just add the class â€” the CSS rule for .email-lib-fullscreen .modal-content
  ...
  // Defer the opener â€” at this point in init, the modules whose handlers
  ...
  // "+" on the Library row â†’ create a new blank document and open it in the
  ...
  // Manage Chats â€” opens Full Library modal (decoupled from Chats accordion toggle)
  ...
  // Sidebar user bar â€” settings, admin, profile
  ...
  // Fetch auth status â€” populate user bar and show admin button if admin
    ...
    .then(d => {
      ...
      if (userBarName && d.username) {
        ...
        if (displayName.includes('@')) {
          ...
          displayName = local.charAt(0) + 'â€¢â€¢â€¢@â€¢â€¢â€¢â€¢' + ext;
        }
  ...
  if (sortBtn && sortDropdown) {
    ...
    // Sort mode options (newest, oldest, last active) â€” toggleable
    ...
    // AI auto-sort â€” spinner on the sort button itself. Used by both
    // the main "â˜… Tidy" button (AI) and the sub-row "Tidy" button
    ...
    async function _runTidy(skipLlm) {
      ...
      try {
        ...
        if (data.status === 'ok') {
          sessionModule.setSortMode(null); // clear sort â€” tidy creates manual folder order
          _syncSortChecks();
          if (skipLlm) {
            ...
            if (data.updated > 0) {
              ...
              if (remaining > 0) msg += ` â€” ${remaining} unfiled left, hit Tidy again`;
            } else if (remaining > 0) {
              msg = `${remaining} unfiled chats â€” hit Tidy again`;
            } else {
              msg = 'All sorted';
            }
  ...
  // Feature visibility â€” hide admin-disabled features
  ...
  window._initFeaturesReady = (_prefetchedFeatures
    ...
    : fetch(`${API_BASE}/api/auth/features`, { credentials: 'same-origin' }).then(r => r.json())
      ...
      // features fetch finishes hiding things â€” otherwise an admin-
  ...
  window._initSettingsReady = (_prefetchedSettings
    ...
    : fetch(`${API_BASE}/api/auth/settings`, { credentials: 'same-origin' }).then(r => r.json())
      ...
      // NOTE: image_gen_enabled only governs *generating* images in chat â€” the
...
// Voice recording is handled by the dual-purpose send/mic button (see below)
  ...
  // â”€â”€ Toggle persistence â€” delegates to Storage module â”€â”€
  ...
  // â”€â”€ Agent / Chat mode toggle â”€â”€
  ...
  // â”€â”€ Checkbox-backed toggle buttons (with per-mode persistence) â”€â”€
  function setupToggle(btnId, checkboxId, stateKey) {
    ...
    btn.addEventListener('click', () => {
      ...
      // Web search and Research are mutually exclusive â€” Research takes priority
  ...
  // Code Mode toggle wiring
  const codeModeBtn = el('code-mode-toggle-btn');
  const codeModeChk = el('code-mode-toggle');
  if (codeModeBtn && codeModeChk) {
    const saved = !!loadToggleState().code_mode;
    codeModeChk.checked = saved;
    codeModeBtn.classList.toggle('active', saved);
    codeModeBtn.setAttribute('aria-pressed', String(saved));
    document.body.classList.toggle('code-mode-active', saved);

    // Disable/Lock function for Chat button in Code Mode
    const syncChatBtnForCodeMode = (isActive) => {
      const chatBtn = el('mode-chat-btn');
      if (chatBtn) {
        if (isActive) {
          chatBtn.style.pointerEvents = 'none';
          chatBtn.style.opacity = '0.5';
          chatBtn.title = 'Disabled in Code Mode';
        } else {
    ...
    // Disable/Lock function for Research buttons in Code Mode
    const syncResearchBtnForCodeMode = (isActive) => {
      const ids = ['research-toggle-btn', 'overflow-research-btn', 'tool-research-btn'];
      ids.forEach(id => {
        const btn = el(id);
        if (btn) {
          if (isActive) {
            ...
            btn.title = 'Disabled in Code Mode';
          } else {
    ...
    // If Code Mode was saved as active, force-enable Agent mode on load
    if (saved) {
      setTimeout(() => {
        // Turn off research if active since Code Mode is on
        const resChk = el('research-toggle');
        if (resChk && resChk.checked) {
          _syncResearchIndicator(false);
        }
        ...
        syncChatBtnForCodeMode(true);
        syncResearchBtnForCodeMode(true);
      }, 50);
    }

    codeModeBtn.addEventListener('click', () => {
      codeModeChk.checked = !codeModeChk.checked;
      codeModeBtn.classList.toggle('active', codeModeChk.checked);
      codeModeBtn.setAttribute('aria-pressed', String(codeModeChk.checked));
      document.body.classList.toggle('code-mode-active', codeModeChk.checked);
      const st = loadToggleState();
      st.code_mode = codeModeChk.checked;
      saveToggleState(st);

      if (codeModeChk.checked) {
        // Turn off research if active since Code Mode is turning on
        const resChk = el('research-toggle');
        if (resChk && resChk.checked) {
          _syncResearchIndicator(false);
        }
        ...
        syncChatBtnForCodeMode(false);
        syncResearchBtnForCodeMode(false);
      }

      if (uiModule && uiModule.showToast) {
        uiModule.showToast(`Code Mode ${codeModeChk.checked ? 'enabled' : 'disabled'}`, 1800);
      }
    });
  }
  ...
  // â”€â”€ RAG toggle (overflow + indicator) â”€â”€
  ...
  // first external caller â€” group.js / sessions.js fire it before it has ever
  // run locally â€” finds it instead of silently no-op'ing (the "group indicator
  ...
  // â”€â”€ Overflow "..." menu (Research) â”€â”€
  ...
  // â”€â”€ Prevent toolbar buttons from stealing focus (avoids mobile keyboard bounce) â”€â”€
  const chatInputBar = document.querySelector('.chat-input-bar');
  // â”€â”€ Keep textarea focused when interacting with chat bar controls (mobile keyboard fix) â”€â”€
  const _msgTextarea = el('message');
  if (chatInputBar && _msgTextarea) {
    let _refocusOnBlur = false;
    function _flagRefocus(e) {
      if (e.target.closest('textarea, input')) return;
      // Don't refocus for attach â€” file picker needs full focus control
      if (e.target.closest('#overflow-attach-btn')) return;
      // Don't refocus for model picker button â€” focus should go to picker search input
      if (e.target.closest('.model-picker-btn')) return;
      // Don't refocus when tapping the +/chevron tools button â€” the user
    ...
    // Overflow menu is position:fixed â€” may not bubble through chatInputBar on mobile
  ...
  (function initOverflowMenu() {
    ...
    // containing block for `position: fixed` descendants â€” so this menu gets
    ...
    plusBtn.addEventListener('click', (e) => {
      ...
      // .hidden â€” route through closeOverflowMenu so the second-click
    ...
    // box â€” keeps the mobile keyboard up.
    ...
    if (researchBtn) {
      ...
      researchBtn.addEventListener('click', () => {
        ...
        if (turningOn) {
          ...
          // Clear character â€” mutually exclusive with research
          if (presetsModule && presetsModule.deactivateCharacter) presetsModule.deactivateCharacter();
          ...
          // Research requires chat mode â€” force switch from agent
  ...
  // â”€â”€ Auto-collapse toolbar buttons into overflow when space is tight â”€â”€
  (function initToolbarOverflow() {
    ...
    // Buttons that can be collapsed (in reverse priority â€” last collapsed first)
    const collapsibleIds = ['bash-toggle-btn', 'web-toggle-btn'];
    const collapsibleBtns = collapsibleIds.map(id => el(id)).filter(Boolean);
    // Map of toolbar btn id â†’ overflow mirror element (created dynamically)
  ...
  // â”€â”€ Auto-hide model picker when textarea area is too narrow â”€â”€
  (function initModelPickerResponsive() {
    ...
    function checkPickerOverflow() {
      // Skip responsive collapse on mobile â€” keyboard open/close causes flicker
      if (_isMobile) return;
      ...
      if (textarea) {
        textarea.setAttribute('placeholder', w < PLACEHOLDER_HIDE_WIDTH ? '' : 'Message Spark...');
      }
      // Hide entire bottom toolbar (tools, mode toggle) â€” only send button remains
      if (inputBottom) {
  ...
  // â”€â”€ Compare indicator (sidebar only, no overflow) â”€â”€
  ...
  // â”€â”€ Overflow Research toggle â”€â”€
  ...
  // â”€â”€ Overflow Group Chat toggle â”€â”€
  ...
  if (overflowGroupBtn) {
    overflowGroupBtn.addEventListener('click', async () => {
      ...
      if (turningOn) {
        ...
        // Start group â€” create participant sessions immediately
        ...
        uiModule.showToast(`Group chat ready â€” ${picked.length} models`);
      } else {
  ...
  // â”€â”€ Group toggle button (chatbox indicator) â€” click to deactivate â”€â”€
  ...
  // â”€â”€ Incognito mode toggle (on welcome screen) â”€â”€
  ...
  if (incognitoBtn) {
    ...
    incognitoBtn.addEventListener('click', () => {
      // Don't toggle mid-chat â€” incognito only changeable from welcome screen
      const ws = el('welcome-screen');
      ...
      const tipEl = el('welcome-tip');
      incognitoBtn.title = chk.checked ? 'Disable Nobody mode' : 'Enable Nobody mode â€” no memory, no history saved';
      const welcomeName = document.querySelector('.welcome-name');
      if (chk.checked) {
        ...
        if (welcomeName) {
          ...
          // Restart the Lâ†’R clip-wipe reveal on the new label
        ...
        // ephemeral â€” their agent-mode defaults must come back on toggle-off.
        ...
        if (welcomeName && welcomeName.dataset.originalHtml) {
          ...
          // Restart the Lâ†’R clip-wipe reveal on the restored label
          welcomeName.style.animation = 'none';
          welcomeName.offsetHeight;
          welcomeName.style.animation = '';
        }
  ...
  // Incognito indicator click â€” deactivate incognito
  ...
  // â”€â”€ Deactivate incognito mode (called on new session) â”€â”€
  ...
  const UI_VIS_MAP = {
    ...
    // Per-tool visibility â€” fine-grained control over which entries show
  ...
  document.querySelectorAll('.rearrange-toggle').forEach(toggle => {
    toggle.addEventListener('click', () => {
      ...
      // Close the dropdown the toggle lives in â€” the sort dropdown's own
  ...
  // Esc exits rearrange mode (no matter where focus/mouse is) â€” matches the
  ...
  // â”€â”€ Text-only emoji conversion â”€â”€
  ...
  // Common emoji â†’ text description map
  const EMOJI_MAP = {
    'ðŸ˜€':'grinning','ðŸ˜ƒ':'smiley','ðŸ˜„':'smile','ðŸ˜':'grin','ðŸ˜†':'laughing','ðŸ˜…':'sweat smile',
    'ðŸ¤£':'rofl','ðŸ˜‚':'joy','ðŸ™‚':'slightly smiling','ðŸ™ƒ':'upside down','ðŸ˜‰':'wink',
    'ðŸ˜Š':'blush','ðŸ˜‡':'innocent','ðŸ¥°':'smiling hearts','ðŸ˜':'heart eyes','ðŸ¤©':'star struck',
    'ðŸ˜˜':'kissing heart','ðŸ˜—':'kissing','ðŸ˜š':'kissing closed eyes','ðŸ˜™':'kissing smiling eyes',
    'ðŸ¥²':'smiling tear','ðŸ˜‹':'yum','ðŸ˜›':'tongue','ðŸ˜œ':'winking tongue','ðŸ¤ª':'zany',
    'ðŸ˜':'squinting tongue','ðŸ¤‘':'money mouth','ðŸ¤—':'hugging','ðŸ¤­':'hand over mouth',
    'ðŸ¤«':'shushing','ðŸ¤”':'thinking','ðŸ«¡':'saluting','ðŸ¤':'zipper mouth','ðŸ¤¨':'raised eyebrow',
    'ðŸ˜':'neutral','ðŸ˜‘':'expressionless','ðŸ˜¶':'no mouth','ðŸ«¥':'dotted line face',
    'ðŸ˜':'smirk','ðŸ˜’':'unamused','ðŸ™„':'eye roll','ðŸ˜¬':'grimacing','ðŸ¤¥':'lying',
    'ðŸ˜Œ':'relieved','ðŸ˜”':'pensive','ðŸ˜ª':'sleepy','ðŸ¤¤':'drooling','ðŸ˜´':'sleeping',
    'ðŸ˜·':'mask','ðŸ¤’':'thermometer','ðŸ¤•':'head bandage','ðŸ¤¢':'nauseated','ðŸ¤®':'vomiting',
    'ðŸ¥µ':'hot','ðŸ¥¶':'cold','ðŸ¥´':'woozy','ðŸ˜µ':'dizzy','ðŸ¤¯':'exploding head',
    'ðŸ¤ ':'cowboy','ðŸ¥³':'party','ðŸ¥¸':'disguised','ðŸ˜Ž':'sunglasses','ðŸ¤“':'nerd',
    'ðŸ§':'monocle','ðŸ˜•':'confused','ðŸ«¤':'diagonal mouth','ðŸ˜Ÿ':'worried','ðŸ™':'slightly frowning',
    'ðŸ˜®':'open mouth','ðŸ˜¯':'hushed','ðŸ˜²':'astonished','ðŸ˜³':'flushed','ðŸ¥º':'pleading',
    'ðŸ¥¹':'holding back tears','ðŸ˜¦':'frowning open mouth','ðŸ˜§':'anguished','ðŸ˜¨':'fearful',
    'ðŸ˜°':'anxious sweat','ðŸ˜¥':'sad relieved','ðŸ˜¢':'crying','ðŸ˜­':'sobbing','ðŸ˜±':'screaming',
    'ðŸ˜–':'confounded','ðŸ˜£':'persevering','ðŸ˜ž':'disappointed','ðŸ˜“':'downcast sweat',
    'ðŸ˜©':'weary','ðŸ˜«':'tired','ðŸ¥±':'yawning','ðŸ˜¤':'triumph','ðŸ˜¡':'pouting',
    'ðŸ˜ ':'angry','ðŸ¤¬':'swearing','ðŸ˜ˆ':'smiling devil','ðŸ‘¿':'angry devil',
    'ðŸ’€':'skull','â˜ ï¸':'skull crossbones','ðŸ’©':'poop','ðŸ¤¡':'clown','ðŸ‘¹':'ogre','ðŸ‘º':'goblin',
    'ðŸ‘»':'ghost','ðŸ‘½':'alien','ðŸ‘¾':'space invader','ðŸ¤–':'robot',
    'ðŸ˜º':'smiling cat','ðŸ˜¸':'grinning cat','ðŸ˜¹':'tears of joy cat','ðŸ˜»':'heart eyes cat',
    'ðŸ˜¼':'wry cat','ðŸ˜½':'kissing cat','ðŸ™€':'weary cat','ðŸ˜¿':'crying cat','ðŸ˜¾':'pouting cat',
    'ðŸ™ˆ':'see no evil','ðŸ™‰':'hear no evil','ðŸ™Š':'speak no evil',
    'ðŸ‘‹':'wave','ðŸ¤š':'raised back of hand','ðŸ–ï¸':'hand with fingers splayed','âœ‹':'raised hand',
    'ðŸ––':'vulcan salute','ðŸ«±':'rightward hand','ðŸ«²':'leftward hand',
    'ðŸ‘Œ':'ok hand','ðŸ¤Œ':'pinched fingers','ðŸ¤':'pinching hand','âœŒï¸':'victory',
    'ðŸ¤ž':'crossed fingers','ðŸ«°':'hand with index finger and thumb crossed',
    'ðŸ¤Ÿ':'love you','ðŸ¤˜':'rock on','ðŸ¤™':'call me','ðŸ‘ˆ':'point left','ðŸ‘‰':'point right',
    'ðŸ‘†':'point up','ðŸ–•':'middle finger','ðŸ‘‡':'point down','â˜ï¸':'index up',
    'ðŸ«µ':'point at viewer','ðŸ‘':'thumbs up','ðŸ‘Ž':'thumbs down','âœŠ':'raised fist',
    'ðŸ‘Š':'fist bump','ðŸ¤›':'left fist','ðŸ¤œ':'right fist','ðŸ‘':'clap','ðŸ™Œ':'raising hands',
    'ðŸ«¶':'heart hands','ðŸ‘':'open hands','ðŸ¤²':'palms up','ðŸ¤':'handshake','ðŸ™':'pray',
    'âœï¸':'writing','ðŸ’…':'nail polish','ðŸ¤³':'selfie','ðŸ’ª':'flexed biceps',
    'â¤ï¸':'red heart','ðŸ§¡':'orange heart','ðŸ’›':'yellow heart','ðŸ’š':'green heart',
    'ðŸ’™':'blue heart','ðŸ’œ':'purple heart','ðŸ–¤':'black heart','ðŸ¤':'white heart',
    'ðŸ©·':'pink heart','ðŸ©µ':'light blue heart','ðŸ©¶':'grey heart','ðŸ¤Ž':'brown heart',
    'ðŸ’”':'broken heart','â¤ï¸â€ðŸ”¥':'heart on fire','â¤ï¸â€ðŸ©¹':'mending heart',
    'ðŸ’•':'two hearts','ðŸ’ž':'revolving hearts','ðŸ’“':'heartbeat','ðŸ’—':'growing heart',
    'ðŸ’–':'sparkling heart','ðŸ’˜':'heart with arrow','ðŸ’':'heart with ribbon',
    'ðŸ’Ÿ':'heart decoration','ðŸ”¥':'fire','ðŸ’¯':'100','âœ¨':'sparkles','â­':'star',
    'ðŸŒŸ':'glowing star','ðŸ’«':'dizzy star','ðŸŽ‰':'party popper','ðŸŽŠ':'confetti ball',
    'ðŸŽˆ':'balloon','ðŸŽ':'gift','ðŸ†':'trophy','ðŸ¥‡':'1st place','ðŸ¥ˆ':'2nd place','ðŸ¥‰':'3rd place',
    'âš¡':'zap','ðŸ’¡':'light bulb','ðŸ”‘':'key','ðŸ”’':'locked','ðŸ”“':'unlocked',
    'ðŸ””':'bell','ðŸ”•':'bell off','ðŸ“¢':'loudspeaker','ðŸ“£':'megaphone',
    'ðŸ’¬':'speech bubble','ðŸ’­':'thought bubble','ðŸ—¯ï¸':'anger bubble',
    'âœ…':'check mark','âŒ':'cross mark','â“':'question','â—':'exclamation',
    'âš ï¸':'warning','ðŸš«':'prohibited','â›”':'no entry','ðŸ”´':'red circle','ðŸŸ¢':'green circle',
    'ðŸ”µ':'blue circle','ðŸŸ¡':'yellow circle','âšª':'white circle','âš«':'black circle',
    'ðŸŸ ':'orange circle','ðŸŸ£':'purple circle','ðŸŸ¤':'brown circle',
    'ðŸ“':'folder','ðŸ“‚':'open folder','ðŸ“„':'document','ðŸ“':'memo','ðŸ“Ž':'paperclip',
    'ðŸ“Œ':'pin','ðŸ“':'round pin','ðŸ”—':'link','ðŸ“Š':'bar chart','ðŸ“ˆ':'chart up','ðŸ“‰':'chart down',
    'ðŸ”':'magnifying glass left','ðŸ”Ž':'magnifying glass right',
    'ðŸŒ':'globe','ðŸŒ':'globe europe','ðŸŒŽ':'globe americas','ðŸŒ':'globe asia',
    'ðŸ•':'clock 1','ðŸ•‘':'clock 2','ðŸ•’':'clock 3','ðŸ•“':'clock 4',
    'â°':'alarm clock','â³':'hourglass flowing','âŒ›':'hourglass done',
    'ðŸš€':'rocket','âœˆï¸':'airplane','ðŸš—':'car','ðŸš‚':'train','ðŸš¢':'ship',
    'ðŸ ':'house','ðŸ¢':'building','ðŸ—ï¸':'construction','ðŸ­':'factory',
    'ðŸŽµ':'musical note','ðŸŽ¶':'musical notes','ðŸŽ¤':'microphone','ðŸŽ§':'headphones',
    'ðŸ“·':'camera','ðŸ“¸':'camera flash','ðŸŽ¬':'clapperboard','ðŸ“º':'television',
    'ðŸ’»':'laptop','ðŸ–¥ï¸':'desktop','ðŸ“±':'mobile phone','â˜Žï¸':'telephone',
    'ðŸ”§':'wrench','ðŸ”¨':'hammer','âš™ï¸':'gear','ðŸ§²':'magnet','ðŸ§ª':'test tube','ðŸ”¬':'microscope',
    'ðŸ“š':'books','ðŸ“–':'open book','âœï¸':'pencil','ðŸ–Šï¸':'pen','ðŸ–‹ï¸':'fountain pen',
    'ðŸŽ¯':'bullseye','â™Ÿï¸':'chess pawn','ðŸŽ²':'game die','ðŸ§©':'puzzle piece',
    'ðŸ•':'pizza','ðŸ”':'burger','ðŸŸ':'fries','ðŸŒ®':'taco','ðŸ£':'sushi','ðŸ©':'donut',
    'â˜•':'coffee','ðŸº':'beer','ðŸ·':'wine','ðŸ¥¤':'cup with straw',
    'ðŸ¶':'dog','ðŸ±':'cat','ðŸ­':'mouse','ðŸ¹':'hamster','ðŸ°':'rabbit','ðŸ¦Š':'fox',
    'ðŸ»':'bear','ðŸ¼':'panda','ðŸ¨':'koala','ðŸ¯':'tiger','ðŸ¦':'lion','ðŸ®':'cow',
    'ðŸ·':'pig','ðŸ¸':'frog','ðŸµ':'monkey','ðŸ”':'chicken','ðŸ§':'penguin','ðŸ¦':'bird',
    'ðŸ¦…':'eagle','ðŸ¦†':'duck','ðŸ¦‰':'owl','ðŸº':'wolf','ðŸ—':'boar','ðŸ´':'horse',
    'ðŸ¦„':'unicorn','ðŸ':'bee','ðŸ›':'bug','ðŸ¦‹':'butterfly','ðŸŒ':'snail','ðŸž':'ladybug',
    'ðŸ':'snake','ðŸ¢':'turtle','ðŸ™':'octopus','ðŸ¦€':'crab','ðŸ ':'tropical fish',
    'ðŸ³':'whale','ðŸ‹':'whale','ðŸ¦ˆ':'shark','ðŸŠ':'crocodile','ðŸ¦•':'sauropod','ðŸ¦–':'t-rex',
    'ðŸŒ¸':'cherry blossom','ðŸŒ¹':'rose','ðŸŒ»':'sunflower','ðŸŒº':'hibiscus','ðŸŒ·':'tulip',
    'ðŸŒ±':'seedling','ðŸŒ²':'evergreen tree','ðŸŒ³':'deciduous tree','ðŸ€':'four leaf clover',
    'ðŸŽ':'red apple','ðŸ':'pear','ðŸŠ':'tangerine','ðŸ‹':'lemon','ðŸŒ':'banana',
    'ðŸ‰':'watermelon','ðŸ‡':'grapes','ðŸ“':'strawberry','ðŸ«':'blueberries','ðŸ‘':'peach',
    'ðŸŒˆ':'rainbow','â˜€ï¸':'sun','ðŸŒ¤ï¸':'sun behind cloud','â›…':'sun behind cloud','â˜ï¸':'cloud',
    'ðŸŒ§ï¸':'rain','â›ˆï¸':'thunder','â„ï¸':'snowflake','ðŸŒŠ':'wave',
    'ðŸ‘€':'eyes','ðŸ‘ï¸':'eye','ðŸ‘‚':'ear','ðŸ‘ƒ':'nose','ðŸ‘„':'mouth','ðŸ‘…':'tongue',
    'ðŸ§ ':'brain','ðŸ¦´':'bone','ðŸ¦·':'tooth','ðŸ‘¶':'baby','ðŸ§’':'child','ðŸ‘¦':'boy','ðŸ‘§':'girl',
    'ðŸ§‘':'person','ðŸ‘¨':'man','ðŸ‘©':'woman','ðŸ§“':'older person',
    'ðŸ‘®':'police officer','ðŸ§‘â€ðŸ’»':'technologist','ðŸ‘¨â€ðŸ’»':'man technologist',
    'ðŸ‘©â€ðŸ’»':'woman technologist',
    'ðŸŽ“':'graduation cap','ðŸ§¢':'billed cap','ðŸ‘‘':'crown','ðŸ’Ž':'gem','ðŸ‘“':'glasses','ðŸ•¶ï¸':'sunglasses',
    'ðŸ©¸':'drop of blood','ðŸ’Š':'pill','ðŸ©¹':'bandage','ðŸ§¬':'dna','ðŸ¦ ':'microbe',
    'â˜¢ï¸':'radioactive','â˜£ï¸':'biohazard','â™»ï¸':'recycling',
    'ðŸ³ï¸':'white flag','ðŸ´':'black flag','ðŸš©':'red flag','ðŸ':'checkered flag',
    'âž¡ï¸':'right arrow','â¬…ï¸':'left arrow','â¬†ï¸':'up arrow','â¬‡ï¸':'down arrow',
    'â†—ï¸':'upper right arrow','â†˜ï¸':'lower right arrow','â†™ï¸':'lower left arrow','â†–ï¸':'upper left arrow',
    'â†©ï¸':'left curve','â†ªï¸':'right curve','ðŸ”„':'counterclockwise','ðŸ”ƒ':'clockwise',
    'âž•':'plus','âž–':'minus','âž—':'division','âœ–ï¸':'multiply','â™¾ï¸':'infinity',
    'â€¼ï¸':'double exclamation','â‰ï¸':'exclamation question',
    'Â©ï¸':'copyright','Â®ï¸':'registered','â„¢ï¸':'trademark',
  };
  ...
  function deEmojify(root) {
    ...
    // Monochrome SVG spans from svgifyEmoji â€” Unicode lives in aria-label only
  ...
  // Observe chat history for new/changed messages â€” de-emojify on the fly
  ...
  (function initUIVisibility() {
    ...
    try {
      ['custom-preset-modal', 'rename-session-modal'].forEach((id) => {
        ...
        // hiddenâ†’visible edge so it never fires mid-drag.
  ...
  // Brain, Theme) â€” we just click the prev/next tab so the existing switch
  ...
  (function initTabSwipe() {
    ...
    document.addEventListener('touchend', (e) => {
      ...
      for (const [barSel, tabSel] of SYSTEMS) {
        ...
        if (idx < 0) idx = 0;
        // Swipe left (dx<0) â†’ next tab; swipe right (dx>0) â†’ previous.
  ...
  // Elastic overscroll (rubber-band bounce) â€” desktop wheel only, on chat-history not container
  ...
  if (railNewSession) {
    railNewSession.addEventListener('click', async () => {
      ...
      // No models at all â€” show welcome screen
  ...
  // Mobile new chat button â€” always go to blank welcome screen.
  ...
  if (mobileNewChat) {
    mobileNewChat.addEventListener('click', () => {
      ...
      // callback â€” a setTimeout breaks the user-gesture chain.
      const _input = el('message-input');
      if (_input) { try { _input.focus(); } catch (_) {} }
    });
  }

  // Logo click â†’ new chat (same logic as rail new-session button)
  ...
  // â”€â”€ Ghost text autocomplete for /new and /create commands â”€â”€
...
function startOdysseusApp() {
  ...
  // Smooth keyboard open/close on mobile â€” keep chat scrolled to bottom
  ...
  // Search buttons â€” icon rail + sidebar
  ...
  // Rail tool buttons â€” delegate to sidebar tool buttons
  const _railToolMap = {
  ...
  // Rail chats â€” click to open the completed background session
  ...
  if (_railChatsBtn) {
    _railChatsBtn.addEventListener('click', () => {
      ...
      // Clear notification â€” session will call clearStreamComplete on load
  ...
  // Rail documents â€” toggle doc panel on/off (not library)
  ...
  // â”€â”€ Dual-purpose send/mic button â”€â”€
  ...
  function _updateSendBtnIcon() {
    ...
    if (!hasText && !hasFiles && _isSttEnabled()) {
      ...
      if (isEmptySession) {
        // Already on new chat â€” show arrow in muted style (ready to type)
        sendBtn.innerHTML = _sendIcon;
        ...
        // The button stays a 32px compact icon (no auto-expand to label â€”
    ...
    // Animate icon spin â€” when switching TO newchat or mic (the + or mic
    ...
    // Never animate into send mode (arrow) â€” it should just appear instantly.
  ...
  if (sendBtn) {
    sendBtn.addEventListener('click', (e) => {
      ...
      // New chat mode â€” empty input, no attachments, no STT
  ...
  // â”€â”€ Compare-mode file drop shield â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  // RAG, document editor, â€¦) are unaffected.
  ...
  if (sessionModule) {
    ...
    // Load sessions first (critical path) â€” remove loader when done
    sessionModule.loadSessions()
  ...
  // â”€â”€ Subtle elastic overscroll for sidebar â”€â”€
  if (sidebarInner) {
    const MAX_PULL = 8;
    let _overscroll = 0;
  ...
  // â”€â”€ Global touch-scroll guard for sidebar â”€â”€
  ...
  if (sidebarInner && 'ontouchstart' in window) {
    ...
    }, true); // capture phase â€” intercepts before any child handlers
  }
