...
const _ACTION_RECENTS_KEY = 'spark-msg-actions-recent';
 ...
 */
const _USER_ACTION_RECENTS_KEY = 'spark-user-actions-recent';
...
export function addMessage(role, content, modelName, metadata) {
  try {
      ...
      for (let r = 0; r < maxRound; r++) {
        ...
        if (roundTools.length > 0) {
          ...
          for (const ev of roundTools) {
            ...
            const finalLabel = window.formatToolAction ? window.formatToolAction(ev.tool, ev.command) : ev.tool;
            node.innerHTML = `<div class="agent-thread-dot"></div><div class="agent-thread-header"><span class="agent-thread-icon">${ok ? '\u2713' : '\u2717'}</span><span class="agent-thread-tool">${esc(finalLabel)}</span><span class="agent-thread-status">${ok ? 'done' : 'failed'}</span><span class="agent-thread-chevron">\u25B6</span></div><div class="agent-thread-content">${evCmdHtml}${outHtml}${evDiffHtml}</div>`;
    ...
    var _roleText = role === 'user' ? 'You' : (isSlash || isCompacted) ? 'Spark' : modelRouteLabel(replyModels.requestedModel, resolvedModel);
