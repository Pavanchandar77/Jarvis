...
export function handleUIControl(uiData) {
  ...
  try {
    if (uiEvent === 'toggle' || uiData.ui_event === 'toggle') {
      ...
      document.querySelectorAll('.spark-highlight').forEach(function(e) { e.classList.remove('spark-highlight'); });
      document.querySelectorAll('.spark-hl-label').forEach(function(e) { e.remove(); });
