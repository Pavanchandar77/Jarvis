...
export function _detectBackend(model) {
  ...
  const isColibri = model.architecture === 'glm_moe_colibri' || model.recommended_backend === 'colibri' || _nm.includes('colibri');

  if (isColibri) {
    return { backend: 'colibri', label: 'Colibri' };
  }
...
export function _buildServeCmd(f, modelName, backend) {
  ...
  } else if (backend === 'colibri') {
    const port = f.port || '8080';
    if (_isWindows()) {
      cmd += `$env:COLI_MODEL="${modelName}"; cd "$HOME\\colibri\\c"; python openai_server.py --port ${port}`;
    } else {
      cmd += `export COLI_MODEL="${modelName}" && cd "$HOME/colibri/c" && python3 openai_server.py --port ${port}`;
    }
  ...
  try {
    ...
    const _statusTag = (pkg, isLocal, isSystemDep, winBlocked) => {
      ...
      if (pkg.installed && pkg.pip_update_available === false) {
        const tip = esc(pkg.update_note || pkg.status_note || 'Found externally; update outside Spark.');
    ...
    list.innerHTML = [
      _viewingRemote ? '' : _section('Spark app', 'Run inside the Spark app itself.', _appDeps),
...
function _renderRecipes() {
  ...
  html += '<div class="hwfit-list-footer" style="display:none;">'
       + 'Don\'t see a model? '
       + '<a href="https://github.com/pewdiepie-archdaemon/spark/discussions/1962" target="_blank" rel="noopener" style="color:var(--accent,var(--red));text-decoration:none;display:inline-flex;align-items:center;gap:4px;vertical-align:middle;position:relative;top:-1px;">'
       + 'Request it →'
  ...
  html += '</select>';
  html += '</div>';
  html += '<p class="memory-desc doclib-desc">Optional packages that extend Spark capabilities.</p>';
  ...
  html += '</div>';
  html += '<p class="memory-desc doclib-desc">Configure SSH servers, install Spark keys, choose model directories, and set the default server. Local is this machine.</p>';
