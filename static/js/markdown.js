...
function createThinkingSection(thinkingContent, index = 0, thinkingTime = null) {
  ...
  const isCodeMode = !!state.code_mode;

  let headerLeftHtml = `<span>View thinking process</span>`;
  if (isCodeMode) {
    const timeStr = thinkingTime ? `${thinkingTime}s` : '3s';
    const wordCount = thinkingContent.split(/\s+/).length;
    const tokens = Math.max(12, Math.round(wordCount * 1.3));
    let tokenStr = tokens > 1000 ? `${(tokens/1000).toFixed(1)}k` : tokens;
    
    // Extract first line of the thought content as a summary title
    let lines = thinkingContent.split('\n').map(l => l.trim()).filter(Boolean);
    let summary = lines[0] || 'Analyzing...';
    // Clean up summary string
    summary = summary.replace(/^[#*`_\-\s]+|[#*`_\-\s]+$/g, '').trim();
    ...
    headerLeftHtml = `
      <div style="display:flex; flex-direction:column; gap:2px; text-align:left;">
        <span style="font-weight:600;">▸ Thought for ${escapeHtml(timeStr)}, ${escapeHtml(tokenStr)} tokens</span>
        <span style="font-size:11px; opacity:0.6; padding-left:10px;">${escapeHtml(summary)}</span>
      </div>
    `;
  }

  const timeHtml = (thinkingTime && !isCodeMode) ? `<span style="font-size:11px;opacity:0.4;font-variant-numeric:tabular-nums;">${thinkingTime}s</span>` : '';
  return `
...
export function mdToHtml(src, opts) {
  ...
  // [ASK: Question text?]
  // - [VALUE: Button Text]
  s = s.replace(/\[ASK:\s*([^\]\n]+?)\]\s*([\s\S]*?)(?=(?:\n\n|\n[^\-\s]|$))/g, (_, question, optionsBlock) => {
    let buttonsHtml = '';
    const btnRegex = /-\s*\[([A-Z0-9_-]+):\s*([^\]\n]+?)\]/g;
    let match;
    let index = 0;
    while ((match = btnRegex.exec(optionsBlock)) !== null) {
      const optionVal = match[2].trim();
      const styleClass = index === 0 ? 'question-btn active-accent' : 'question-btn';
      buttonsHtml += `<button type="button" class="${styleClass}" onclick="window.submitInteractiveAnswer('${optionVal.replace(/'/g, "\\'")}')">${escapeHtml(optionVal)}</button>`;
      index++;
    }
    
    if (buttonsHtml) {
      return `
        <div class="interactive-question-card">
          <div class="question-title-row">
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round" style="color:var(--brand-color, var(--red));"><circle cx="12" cy="12" r="10"/><path d="M9.09 9a3 3 0 0 1 5.83 1c0 2-3 3-3 3"/><line x1="12" y1="17" x2="12.01" y2="17"/></svg>
            <span class="question-title">${escapeHtml(question.trim())}</span>
          </div>
...
function initMermaid() {
  if (!window.mermaid || window.__sparkMermaidReady) return;
  window.mermaid.initialize({ startOnLoad: false, theme: 'dark', securityLevel: 'loose' });
  window.__sparkMermaidReady = true;
}
window.sparkInitMermaid = initMermaid;
initMermaid();
...
const THINK_EXPANDED_KEY = 'spark-thinking-expanded';
